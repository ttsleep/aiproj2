...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,CBAM,ResBlock_CBAM,
                 ...
                 Stem_Space_Block,ISF_Net,DWConvblock,SESAC3]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default=ROOT / 'models/yolov5s.yaml', help='model.yaml')
    ...
    if stride == 32:
        input = torch.Tensor(1, 3, 448, 448).to(device)
    else:
        input = torch.Tensor(1, 3, 448, 448).to(device)
    model.train()
    print(model)
    ...
    print("---|---|---")
    print("%s | %.8f | %.8f" % (name, total_params / (1000 ** 2), 2*total_ops / (1000 ** 3)))
